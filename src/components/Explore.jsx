...
const Explore = () => {
    ...
    const fetchPosts = async(pageOpt=page) => {
        const res = await postAPI.getPosts(pageOpt, true);
    ...
    return (
        <div className='h-full w-full bg-[#282c37] rounded-lg flex flex-col'>
            {showPost ? <PostSingle postInfo={showPost} setShowPost={setShowPost}/>
             :  
             <>
                <h3 className='flex w-full h-[10%] sticky border-b-[1px] border-b-[#464b5f] px-4 py-2 items-center bg-[#282c37] rounded-t-lg'>
