      ...
      const observer = new IntersectionObserver(
          ([entry]) => {
              if (entry.isIntersecting) {
                  if (!stopFetching) { // <-- Check if stopFetching is false before fetching more posts
                      fetchPosts();
                  }
              }
          },
    ...
    return (
        <div className={`h-full w-full bg-[#282c37] rounded-lg flex flex-col`}>
            {showPost ? <PostSingle postInfo={showPost} setShowPost={setShowPost}/>
             :  
             <div className='w-full h-full md:grid md:grid-rows-[10%_90%] md:grid-cols-1 '>
                <h3 className='flex w-full h-fit sticky border-b-2 border-b-[#464b5f] px-4 py-2 items-center bg-[#282c37] rounded-t-lg'>
                ...
                <div className=' flex flex-col w-full h-full overflow-y-scroll overflow-x-hidden scrollbar:bg-blue-500 rounded-xl scrollbar scrollbar-thumb-blue-500 scrollbar-track-gray-200'>
                    {user && <PostForm setPosts={setPosts} setShowPost={setShowPost}/>}
                    {errors && <div className='text-red-500 text-center'>{errors}</div>}
                    {posts && posts.length === 0 && <div className='w-full h-full flex-grow flex justify-center items-center text-[#ffffff3f] text-lg'>No posts</div>}
                    {posts.length > 0 && 
                        <>
                            {posts}
                            {isLoading ? 
