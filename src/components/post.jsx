  ...
  return (
    ...
    <div className='w-full p-4 border-[#464b5f] border-b-[1px] text-white flex flex-col cursor-pointer'>
      ...
      <>
        ...
        <div className='w-full max-h-[80vh] flex flex-col text-sm pb-6' onClick={()=>setShowPost(postInfo)}>
          <p className='w-full my-2 text-md'>{decodeURI(postInfo.content)}</p>
          {postInfo.image && <img src={postInfo.image} alt='Post Image' className='w-full h-[95%] object-scale-down rounded-md'/>}
        </div>
        
        <div className='w-full grid grid-cols-2 justify-between'>
          
          <button onClick={handleDelete} className={user && user._id === postInfo.author._id ? 'visible' : 'invisible'}>
