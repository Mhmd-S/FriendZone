...
const Root = () => {
  ...
  return (
    <div className='w-screen h-screen grid grid-rows-[7.5%_85%_7.5%] grid-cols-1 md:grid md:grid-cols-[15%_62.5%_22.5%] md:grid-rows-1 bg-[#191b22]'>
      
      <div className='w-full justify-self-end order-3 md:h-screen flex justify-between items-center md:order-1 md:flex-col '> {/* Navigation bar */}
        <div className='w-full h-full text-[#d9e1e8] flex items-center md:flex-col md:h-2/5'>

          <Link className='hidden w-full pt-4 md:flex justify-center items-center' to={'/home'}>
            <Logo/>
          </Link>
          
          <hr className='hidden border-t-px border-[#464b5f] w-[85%] self-center'/>

          <ul className='w-full h-full text-md flex justify-evenly items-center p-4 md:flex-col md:items-start'>
            <li className='p-2'>
              <Link to='home' className={'flex items-center w-full hover:text-[#595aff] ' + (location.pathname === '/home' ? ' text-[#595aff]' : '')} src='./icons/home.svg'>
                ...
                <span className='hidden pl-4 md:inline'>Home</span>
              </Link>
            </li>
            <li className='p-2'>
              <Link to='search'  className={'flex items-center w-full hover:text-[#595aff] ' + (location.pathname === '/search' ? ' text-[#595aff]' : '')}>
                ...
                <span className='hidden pl-4 md:inline'>Search</span>
              </Link>
            </li>
            <li className='p-2'>
              <Link to='explore' className={'flex items-center w-full hover:text-[#595aff] ' + (location.pathname === '/explore' ? ' text-[#595aff]' : '')}>
                ...
                <span className='hidden pl-4 md:inline'>Explore</span>
              </Link>
            </li>
            <li className='p-2'>
              <Link to='chat' className={'flex items-center w-full hover:text-[#595aff] ' + (location.pathname === '/chat' ? ' text-[#595aff]' : '')}>
                ...
                <span className='hidden pl-4 md:inline'>Chat</span>
              </Link>
            </li>
          </ul>
      ...
      <div className='w-full order-2 overflow-y-hidden rounded-md md:h-screen md:py-3 relative'>
        <Outlet />
      ...
      <div className='w-full flex justify-between px-3 order-1 md:order-3 md:flex-col md:h-screen'>
        
        <Link className='flex justify-center items-center md:hidden' to={'/home'}>
          <Logo />
        </Link>
        ...
        <div className={`${displayMenu ? 'flex' : 'hidden'} flex-col items-center w-full h-screen absolute top-0 left-0 bg-[#191b22] p-4 md:flex md:h-full md:w-full md:relative md:top-auto md:left-auto z-10 md:z-0`}>
          
          <button onClick={()=>setDisplayMenu(false)} className='w-full flex justify-end md:hidden'>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#595aff" className="w-8 h-8">
